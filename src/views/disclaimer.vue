<template>
<div>
  <v-img src="../assets/disclamer.png" max-height="200"></v-img>
   <div id="addDisclaimerDataDiv">
   </div>
...
export default {
  ...
  methods:{
    updateAboutUs(){
     ...
     loadAboutUsData() {
        let ref = db.collection("youtubethumb").doc("disclaimer");
        ref.onSnapshot(res => {
          if (res && res.data()) {
            this.content=res.data().disclaimer;
           let ele= document.getElementById('addDisclaimerDataDiv');
           if(ele){
    ...
    updateAboutUsData(){
     ...
     if (localStorage && localStorage.adminData && localStorage.adminData.length) {
        ...
        let cDate=new Date().getTime();
         let obj={timeStamp:cDate,disclaimer:this.content}
